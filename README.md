...
| **积分查询**           | App 内「查询积分」按钮，遍历全部账号：Trae 展示额度用量（已用 / 总额），WorkBuddy 展示今日签到、本期累计天数与积分；结果以弹窗展示、可长按选中复制                                                      |
| **一键提取 Token**     | `run.cmd` 双击完成「装依赖 → 提取 → 结果自动进剪贴板」，账号切换后一条命令拿到新 Token                                                                                    |
| **签到天数口径**         | 平台侧**没有「连续签到天数」字段**：`streak_days` / `checkin_dates` 都是**本期赛季累计**（活动换期一起归零），所以 App 不显示「连续 N 天」。成长中心的 `streak.days` 是**连续登录 PC 端**的天数、与签到无关 |
...
| 安装到手机         | Android 8.0（API 26）及以上                                                  |
| Token 来源      | 电脑上已登录的 Trae 桌面端 / WorkBuddy 客户端                                        |

> 编译不强制要求 Android Studio，Windows 命令行（含 WSL2）即可（下文以命令行方式说明）。
> 提取 Token 需要 Trae / WorkBuddy 的客户端数据文件。
...
- **Trae**：读取 `storage.json`，取出 `accessToken` / `refreshToken` / 设备号
  - 设备号取的是 `icube-dc:` 键后缀的 **16 位纯数字 Aha 设备号**，
    而**不是**  UUID 格式的旧设备号
  - 服务端按注册指纹校验设备号，填 UUID 会被当成陌生设备、触发更严格的限流（签到报 `9074`）
- **WorkBuddy**：读取 `workbuddy-desktop.info`，
...
3. 后台被国产 ROM 清掉、导致当天**一次都没跑过**时，下次打开 App 补跑一次（一天最多一次；结果照常通知，成功或「今日已签到」都会推送）
...
- **失败通知不刷屏**：Trae 遇限流会按固定节奏重试（10 分钟一轮、最多 3 次），失败通知只在「不再重试」的那一轮发出；签到成功（含「今天本来就已签」）则**每次都通知**——这是确认定时任务真的跑了的主要信号

***

## Token 生命周期与刷新

以下数值来自对本机桌面客户端凭据的实测，官方未公开文档承诺，平台更新可能调整：
...
**Q：定时签到总不跑 / 后台被杀？**App 没有常驻后台服务，靠 WorkManager 定时唤醒；但国产 ROM 的省电策略可能把定时任务连根清掉（尤其息屏数小时后），WorkManager 的「最终执行」保证在这类强杀面前会失效。建议手动给本 App 放行：

- **自启动**：系统设置 → 应用管理 → Auto Checkin → 允许「自启动」
- **电池策略**：改为「无限制 / 不优化」，不要放进深度休眠或省电严格档
- **后台运行**：允许「后台活动 / 后台弹出界面」；多任务界面里下拉锁定卡片更稳
- **通知权限**：Android 13+ 单独一项运行时授权，见上一条
- 兜底机制：即使任务被清掉，当天**一次都没跑过**时下次打开 App 会自动补签一次（一天最多一次）；当天已经跑过（哪怕失败）就不再补，由通知提示手动签到。

**Q：提示「请重新提取 Token」？**
说明 Trae 的 refreshToken 刷新被服务端拒绝了。在电脑上重新跑一次 `run.cmd`，把新产出的 `checkin_auth.json` 粘贴回 App 即可。

**Q：Trae 提示「当前参与用户太多，请稍后重试」？**
这是 Trae 服务端**限流**（错误码 `9074`）。**首要看** **`x-device-id`** 是否正确

- **`9074`** **与设备号取值强相关**：只有客户端**注册过的**那个 16 位 Aha 号
...
**Q：Trae 多个账号能同时签到吗？**
Trae **同一台设备每天只有第一个账号能签成**，这是平台的设备级去重（返回 `9095`
「当前设备今日已经签到，请明日再来哦～」，此时该账号自己的 `checked_in` 仍是 `false`，
即**账号没签、是设备的名额被占了**）。

- **每个账号要配「它自己那台机器」的设备号**，把 A 机的设备号配给 B 机提取的账号，就会撞上设备级去重
- 同一台机器提取的多个账号，每天只有第一个能签成
- 多账号要都签上，现实解法是**一个账号配一台设备**
...
**Q：接口突然开始报错？**
两平台接口均来自社区，客户端 / 服务端更新后可能失效，需要跟进社区实现更新。
